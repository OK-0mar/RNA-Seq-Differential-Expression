...
# 4. Explore if is There any Missing Expression Value

sum(is.null(count))


# 5. Exploring Data Distribution Using Histogram Plot

hist(x = count, col = "orange", main = "Histogram")


# 6. Scaling the Data using log2 Transformation for Better Visualization
# using (+1) to avoid the infinity character when we log zero value

hist(x = log2(count+1), col = "orange", main = "Histogram")


# 7. Filter Low Count Genes Which have Row Mean Lower Than 1

count <- count[rowMeans(count) > 1,]

# 8. Calculating Fold Change
...
# 9. View the Distribution of Fold Change

hist(fold)


# 10. Doing the Differential Expression Statistical Testing
...
# 11. Making the Hypothesis Testing using T test for Each Row (gene)

t <- rowttests(count, phenotype$grouping)


# 12. Correct the T test P value using FDR method

p_adj <- p.adjust(t$p.value, method = "fdr")


# 13. Save the Result in a Data Frame Contain the Fold Change and P Adjusted value

result <- as.data.frame(cbind(fold, p_adj))


# 14. Choose the Statstical Significant Differentaily Expressed Genes (DEGs)
